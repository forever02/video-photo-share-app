...
float IMAGE_MIN_HEIGHT = 1000;
float IMAGE_MIN_WIDTH = 1000;
...
- (void)viewDidLoad
{
    ...
    if (self){
        ...
        CGRect view = CGRectMake(0, statusBarSize.height + [[self navigationController] navigationBar].bounds.size.height, self.view.bounds.size.width, self.view.bounds.size.height - [[self navigationController] navigationBar].bounds.size.height - statusBarSize.height);
        ...
        [self.cropView handleDragCropArea1];
...
static CGFloat const DEFAULT_CONTROL_POINT_SIZE = 20;
...
- (void)handleDragCropArea1{
    ...
    CGFloat cropAreaWidth = imageFrameInView.size.width;
    CGFloat cropAreaHeight = imageFrameInView.size.width;
...
- (void)initViews {
    ...
    int initialCropAreaSize = self.frame.size.width / 2;
...
//    [self boundingBoxForTopLeft:topLeftPoint.center bottomLeft:bottomLeftPoint.center bottomRight:bottomRightPoint.center topRight:topRightPoint.center];
    CGFloat minValue;
    if(blur.size.width > blur.size.height){
        minValue =blur.size.height;
    }else{
        minValue =blur.size.width;
        
    }
//    CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = CGRectMake(0, 0, minValue, minValue);
    cropAreaView.frame = cropArea;
    cropArea.origin.y = cropArea.origin.y - imageFrameInView.origin.y;
    cropArea.origin.x = cropArea.origin.x - imageFrameInView.origin.x;
    [self.shadeView setCropArea:cropArea];
