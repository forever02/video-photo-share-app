...
-(void)defaultSettings
{
    ...
    CGFloat bottomHeight = IS_IPHONE_4 ? 45.0f : 55.0f;
    CGRect rect = CGRectMake(0, self.frame.size.height - bottomHeight, self.frame.size.width, 20);
    
    _pageControl = [[UIPageControl alloc]initWithFrame:rect];
    _pageControl.pageIndicatorTintColor = RGB(188, 190, 192, 1);
    _pageControl.currentPageIndicatorTintColor = RGB(143, 206, 220, 1);
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (nextPage > 0 && nextPage < _viewsToSublime.count + 1) {
        ...
        [self animationWithImageNumber:currentPage - 1 value:1 - percentageScrolledOnCurrentPage];
        [self animationWithImageNumber:nextPage - 1 value:percentageScrolledOnCurrentPage];

    }
    
    if (currentPage == _viewsToSublime.count && scrollView.contentOffset.x > self.lastXOffset + 50) {
        if ([self.delegate respondsToSelector:@selector(lgSublimationViewDidEndScrollingLastPage:)]) {
            [self.delegate lgSublimationViewDidEndScrollingLastPage:self];
        }
    }
}


- (void)animationWithImageNumber:(int)position value:(CGFloat)value;
{
    UIImageView *imageView = _viewsToSublime[position];
    CGRect circleFrame = CGRectMake(-20,
                                    -20,
                                    CGRectGetWidth(imageView.frame) + 40,
                                    CGRectGetHeight(imageView.frame) + 40);
    ...
    CAShapeLayer *shapeLayer = self.shapeLayerArray[position];
    if (!shapeLayer) {
        
        shapeLayer = [CAShapeLayer layer];
        shapeLayer.path = path.CGPath;
        shapeLayer.anchorPoint = circleAnchor;
        shapeLayer.frame = CGRectMake(0,
                                      0,
    ...
    shapeLayer.transform = CATransform3DMakeScale(value, value, value);
}
